...
| Load testing | `scripts/load-test.mjs` (keep-alive HTTP) |
...
scripts/load-test.mjs         # keep-alive load test with a variable request count
dashboard/index.html          # load-test UI (charts of the last 5 runs)
docker-compose.yml            # 3 gateways + Redis + Nginx + Prometheus + Grafana
```
...
`scripts/load-test.mjs` sends keep-alive HTTP requests. Unless you pass
`--clients` or `--concurrency`, both scale with the request count:

- **clients** = `ceil(count / 5)`, capped at 1000. Each client gets its own
  `X-Forwarded-For` IP and `x-api-key`.
- **concurrency** = 10 (≤100 requests), 25 (≤1k), 50 (≤10k), 100 (≤50k),
  otherwise 200.

So `npm run loadtest -- 100` uses **20 clients**. Traffic is also mixed
across the three routes (~50% search, ~30% ping, ~20% orders). That is
only ~2–3 `/api/search` hits per IP against a burst of 10, so every
request can return `200` even though the limiter is working
(`failOpen: 0` on `/stats`).
...
```bash
# scale up; clients and concurrency auto-scale
...
npm run loadtest -- 1000 --clients 50 --concurrency 25

# ladder in one shot (prints a comparison table)
npm run loadtest -- --steps 100,1000,10000,50000,1000000

# docker-compose cluster (Nginx)
npm run loadtest -- 10000 --port 8080
```

Each run prints throughput, HTTP 200 vs 429, and p50/p95/p99 latency,
curls `GET /stats`, and **appends** the summary to `load-test-results.json`
(keeps the last 5 runs: `{ "runs": [ ... ] }`). Older file shapes (a
single run object) are migrated on the next write.
